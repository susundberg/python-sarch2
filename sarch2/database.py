...
class Database():
    ...
    def get_iterator( self, path ):
        ...
        with self.db_cursor( ) as c:
            ...
            if path == "" or path == ".":
                ...
                c.execute("SELECT * FROM files WHERE filename LIKE ?",( path + r"%" , ) )
    ...
    def add( self, file_info ):
        with self.db_cursor( ) as c:
            c.execute("INSERT INTO files values (?,?,?,?,?,?)", ( str(file_info.name), file_info.size, file_info.timestamp, file_info.checksum, STATUS_OK, int(time.time())) )
    ...
    def update( self, file_info, status = None ):
        ...
        with self.db_cursor( ) as c:
             ...
             values = [ file_info.size, file_info.timestamp, file_info.checksum, int(time.time()), ]
             if status != None:
                cmd += ", status=? " 
    ...
    def remove( self, filename ):
        ...
        with self.db_cursor( ) as c:
            c.execute("UPDATE files SET status=?, db_time=? WHERE filename=?",( STATUS_DEL, int(time.time()), str(filename),) )
            assert( c.rowcount == 1 )    
