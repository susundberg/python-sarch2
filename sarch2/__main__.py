...
''')
    ...
    def init_cmd( self ):
        parser = argparse.ArgumentParser( description='Creates REPO to curren path' )
        return parser
    ...
    def save_cmd( self ):
        parser = argparse.ArgumentParser( description='Saves current filesystem to REPO' )
        parser.add_argument('--path', nargs='+', help="Optional path to work with, if skipped uses repository base-path", default=["",])
        parser.add_argument('--really', action='store_true', help="Resolve conflicts" )
        return parser
    ...
    def save_exe( self, config, extra ):
        ...
        if extra:
            config.path = extra
        with self._open_db_and_cwd( False ):
            worker = commands.WorkerSave( repo = self.repo, resolves = config.really )
    ...
    def status_cmd( self  ):
        parser = argparse.ArgumentParser( description='Shows current REPO status' )
        parser.add_argument('--verify', action="store_true", help="Verify contents (checksum) in each file")
        parser.add_argument('--path', nargs='+', help="Optional path to work with, if skipped uses repository base-path", default=["",])
        return parser
    ...
    def status_exe( self, config, extra ):
        ...
        with self._open_db_and_cwd( False ):
            paths = filesystem.make_relative( config.path )
            for path in paths:
               commands.full_scan( path = path, worker = worker, repo = self.repo, flag_verify = config.verify  )
        return worker.status_ok()
        
    def import_cmd( self ):
        parser = argparse.ArgumentParser( description='Import files from given path' )
        parser.add_argument('path', nargs=1, help="What path to import")
        parser.add_argument('--dry-run', action='store_true', help="Do not import but print what would happen" )
        return parser
    ...
    def import_exe( self, config ):
        ...
        with self._open_db_and_cwd( True ):
            for path in paths:
                commands.full_scan_import( worker=worker, path=path, repo=self.repo )
